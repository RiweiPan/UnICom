...
// helper function to study fragmentation in the ext4 fs imafe
// it counts the number of free blocks and is called by sysfs interface
// however we do not use it and TODO:REMOVE
void swiftcore_ext4_count_free_clusters(struct seq_file *m, struct super_block *sb)
{
	ext4_fsblk_t desc_count;
	struct ext4_group_desc *gdp;
	ext4_group_t i;
	...
	for (i = 0; i < ngroups; i++) {
		...
		x = ext4_count_free(bitmap_bh->b_data,
				    EXT4_CLUSTERS_PER_GROUP(sb) / 8);
		seq_printf(m, "group %u: stored = %d, counted = %u\n",
			i, ext4_free_group_clusters(sb, gdp), x);
		bitmap_count += x;
	}
	brelse(bitmap_bh);
	seq_printf(m,"ext4_count_free_clusters: stored = %llu"
	       ", computed = %llu, %llu\n",
	       EXT4_NUM_B2C(EXT4_SB(sb), ext4_free_blocks_count(es)),
	       desc_count, bitmap_count);
  return;
}
EXPORT_SYMBOL(swiftcore_ext4_count_free_clusters);
#endif
