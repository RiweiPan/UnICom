...
struct ext4_inode_info {
	...
	struct rb_root swiftcore_vma_tree; // vma tree holding the processes' virtual ranges that have opened the file via swiftcore (and thus map it)
	unsigned long long num_swiftcore_vmas; // the number of entries on the above tree
	void *pgd; // the root of the file table tree 
	unsigned long pt_ceiling;   // pt_ceiling and pt_level depict the depth of the file table tree 
								//-- it is built bottom up for storage purposes and thus could be shorter than 4-level 
								// (e.g. only 1 PTE leaf for files < 2MB)
	u16 pt_level;
#endif
#ifdef CONFIG_NAP_MAP_SUPPORT
	struct nm_tree *nm_tree;
#endif
};

#ifdef CONFIG_SWIFTCORE_SUPPORT
// a helper swiftcore vma struct that combines the process's original vma (that maps the file via swiftcore) 
// with a helper vma that maps the file tables themselves used only for PoC.
struct swiftcore_vma_item {
  struct rb_node node;
  struct vm_area_struct *vma;
  struct vm_area_struct *vma_expose;
};
#endif
...
#ifdef CONFIG_SWIFTCORE_SUPPORT
	bool ext4_swiftcore_page_tables; //bool to turn on/off file table creation
#endif
#ifdef CONFIG_NAP_MAP_SUPPORT
	int ext4_nap_map_enable;
...
#ifdef CONFIG_NAP_MAP_SUPPORT
extern void ext4_init_nm_ops(struct ext4_sb_info *sbi);
