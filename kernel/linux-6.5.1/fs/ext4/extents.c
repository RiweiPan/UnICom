...
got_allocated_blocks:
	...
	if (sbi->ext4_swiftcore_page_tables) {
		...
		//built the file table entries for the newly allocated blocks
		ext4_swiftcore_build_tables(handle, inode, map->m_len, map->m_pblk, map->m_lblk, 1);
		unsigned long unaligned = map->m_lblk % 512; 
		unsigned long aligned = map->m_lblk / 512;
		
		//if this results in new file table pages -- map them silently to apps that have opened the file via swiftcore (and thus already map it). 
		// This assumes that the initally allocated virtual range for the file can hold the etxension (see open.c)
		if((sih->num_swiftcore_vmas > 0)) {
			
			if (unaligned && ((map->m_len + unaligned) <= 512))
				goto out;
			
			if (unaligned) 
				aligned++;
			
			ext4_swiftcore_subattach_tables(inode, aligned * 512, map->m_len - unaligned);
		}
  	}
#endif
#ifdef CONFIG_NAP_MAP_SUPPORT
	if (sbi->ext4_nap_map_enable && EXT4_I(inode)->nm_tree) {
		update_nm_tree(EXT4_I(inode)->nm_tree, map->m_lblk, map->m_len, map->m_pblk);
	}
#endif
	ext4_ext_show_leaf(inode, path);
out:
...
#if defined(CONFIG_SWIFTCORE_SUPPORT) || defined(CONFIG_NAP_MAP_SUPPORT)
...
// traverse the extent tree and built the swiftcore file tables
// used during cold open operations for already existing files
void swiftcore_build_tables_from_tree(struct inode *inode)
{
	struct ext4_ext_path *path;
	int depth = ext_depth(inode);
	struct ext4_extent_header *eh;
	...
	for (i = 0; i < le16_to_cpu(eh->eh_entries); i++, ex++) {
		ext4_swiftcore_build_tables(NULL, inode, ext4_ext_get_actual_len(ex), ext4_ext_pblock(ex), le32_to_cpu(ex->ee_block), 1);
	}
}

//Profiling -- debugging. It prints the leafs of the extent tree (via sysfs). We do not use it. TODO:REMOVE
void swiftcore_ext4_ext_show_leaf(struct seq_file *m,struct inode *inode)
{
	struct ext4_ext_path *path;
	int depth = ext_depth(inode);
	struct ext4_extent_header *eh;
	...
	for (i = 0; i < le16_to_cpu(eh->eh_entries); i++, ex++) {
		seq_printf(m,"logical blocks: 0x%llx-0x%llx; physical blocks: 0x%llx-0x%llx; len: %llu(blocks) %llu(KB)\n", 
		le32_to_cpu(ex->ee_block),
		le32_to_cpu(ex->ee_block)+ext4_ext_get_actual_len(ex),
		ext4_ext_pblock(ex),
		ext4_ext_pblock(ex)+ext4_ext_get_actual_len(ex),
		ext4_ext_get_actual_len(ex),
		ext4_ext_get_actual_len(ex)*4);
	}
	seq_printf(m,"\n");
}
EXPORT_SYMBOL(swiftcore_ext4_ext_show_leaf);
#endif
