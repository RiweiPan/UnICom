...
stop_handle:
	...
	if(EXT4_I(inode)->nm_tree) {
		free_nm_tree(EXT4_I(inode)->nm_tree);
		EXT4_I(inode)->nm_tree = NULL;
	}
#endif
...
#ifdef CONFIG_SWIFTCORE_SUPPORT
	//delete the swiftcore file tables when the file shrinks (truth: not sure we call it always when we should -- in every case)
  	struct ext4_sb_info *sbi = EXT4_SB(inode->i_sb);
	if (sbi->ext4_swiftcore_page_tables) {
    	ext4_swiftcore_delete_tables(handle, inode, inode->i_sb, round_up(inode->i_size, PAGE_SIZE), 0xffffffffff, 1);
	}
#endif
#ifdef CONFIG_NAP_MAP_SUPPORT
	if (sbi->ext4_nap_map_enable && ei->nm_tree) {
		update_nm_tree(ei->nm_tree, round_up(inode->i_size, PAGE_SIZE), NAP_MAP_MAX_EXTENT_LEN, 0);
	}
#endif
