...
static struct inode *ext4_alloc_inode(struct super_block *sb)
{
	...
	mutex_init(&ei->i_fc_lock);
 	...
 	//SWIFTCORE metadata per inode
  	ei->swiftcore_vma_tree = RB_ROOT; //the per file vma tree
  	ei->num_swiftcore_vmas = 0; // the number of vmas maping the file via swiftcore
  	ei->pgd=NULL; // the root of the swiftcore file tables
  	ei->pt_level=4; // the depth of the file table tree (reverse) -- it is built bottom-up and sparsely
  	ei->pt_ceiling=0; // the maximum file size the current depth of the tree can handle
#endif
#ifdef CONFIG_NAP_MAP_SUPPORT
	ei->nm_tree = NULL;
#endif
