...
int __v_p4d_alloc_ext4(handle_t *handle, struct inode *inode, pgd_t *pgd, unsigned long address)
{
        p4d_t *new = (p4d_t*) ext4_pt_alloc(handle, inode);
        if (!new)
          return -ENOMEM;
        smp_wmb();
        if(pgd)
          volatile_pgd_populate_ext4(pgd, new);
        return 0;
}
