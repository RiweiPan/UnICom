	...
	do {
		...
		if (pte_present(ptent)) {
			...
			zap_install_uffd_wp_if_needed(vma, addr, pte, details,
						      ptent);
#ifdef CONFIG_SWIFTCORE_SUPPORT
			if (unlikely(!page) || vma->vm_flags & VM_SWIFTCORE_EXPOSE)
				continue;
#else
			if (unlikely(!page))
				continue;
...
#ifdef CONFIG_SWIFTCORE_SUPPORT
		//if it is a swiftcore pmd to not detsroy tables -- do the shootdowns and just de-attach the swiftcore file tables 
    	if(!pmd_none(*pmd) && is_pmd_swiftcore(*pmd)) {
    		struct mm_struct *mm = tlb->mm;
		...
		unsigned long end_addr, bool mm_wr_locked)
{
	...
	do {
#ifdef CONFIG_SWIFTCORE_SUPPORT
		if (vma->vm_flags & VM_SWIFTCORE_EXPOSE_ADDRESS_SPACE)
			continue;
#endif
...
#ifdef CONFIG_SWIFTCORE_SUPPORT
	//if (vma->vm_flags & VM_SWIFTCORE_BUFFER_TO_EXPOSE){
	if (vma->vm_mm->swiftcore_expose_address_space && (vmf->address > (((uint64_t) 0x200000 + SWIFCTORE_EXPOSE_SVA)))){
		pte_t *pte_hack, *phys_pte;
		pte_t entry_hack;
		spinlock_t *ptl_hack;
		struct page *page_hack;

		pte_hack = get_locked_pte(vma->vm_mm,(uint64_t)0x200000+(((((vmf->address-((uint64_t)0x200000+SWIFCTORE_EXPOSE_SVA))/4096)/512)*512)*sizeof(uint64_t)),&ptl_hack);
		
		if(!pte_hack) 
			goto fallthrough_hack;
		
		if(pte_present(*pte_hack)) {
			pte_unmap_unlock(pte_hack, ptl_hack);
			goto fallthrough_hack;
		}
		phys_pte = pte_offset_kernel(vmf->pmd,0);
		page_hack = pfn_to_page(__pa(phys_pte)>>PAGE_SHIFT);
		entry_hack = pfn_pte(__pa(phys_pte)>>PAGE_SHIFT, PAGE_READONLY);	
		entry_hack=pte_mkyoung(entry_hack);
		set_pte(pte_hack, entry_hack);
		pte_unmap_unlock(pte_hack, ptl_hack);
	} else if (vma->vm_mm->swiftcore_expose_address_space ) {
		pr_crit("OOOps we are out of area expose vma is:0x%llx-0x%llx, so covering area is 0x%llx-0x%llx faulting address is 0x%llx  (most likley we fall inside the expose vma shich is scary)!\n",  0x200000, 0x200000+SWIFCTORE_EXPOSE_SVA, 0x200000+SWIFCTORE_EXPOSE_SVA, TASK_SIZE, vmf->address);
	}
fallthrough_hack:
#endif
	/* No need to invalidate - it was non-present before */
